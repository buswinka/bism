...
def eval(image_path: str, model_file: str):
    ...
    testin = torch.rand((1, cfg.MODEL.IN_CHANNELS, 300, 300, 20)) if '3d' in cfg.MODEL.BACKBONE else torch.rand((1, cfg.MODEL.IN_CHANNELS, 300, 300))
    ...
    for slice, ind in iterator:
        ...
        if '3d' in cfg.MODEL.BACKBONE:
            ...
            modelout[:,
            x + overlap[0]: x + cropsize[0] - overlap[0],
            y + overlap[1]: y + cropsize[1] - overlap[1]] = out[0, :,
                                                            overlap[0]: -overlap[0],
                                                            overlap[1]: -overlap[1]].half().cpu()
            iterator.desc = f'Evaluating UNet on slice [x{x}:y{y}]'
