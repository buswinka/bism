...
class CPnet_2D(nn.Module):
    def __init__(self,
        ...
        self.init_stage = nn.Sequential(
            nn.Conv2d(in_channels, dims[0], kernel_size=(7, 7), padding=(3, 3)),
        ...
        stem = nn.Sequential(
            nn.Conv2d(dims[0], dims[0], kernel_size=(2, 2), stride=(2, 2)),
    ...
    def __repr__(self):
        return f'CellPoseNet2D[dims={self.dims}, depths={self.depths}, ' \
               f'in_channels={self.in_channels}, out_channels={self.out_channels}]'


class CPnet_3D(nn.Module):
    def __init__(self,
                 in_channels: Optional[int] = 1,
                 out_channels: int = 4,
                 *,
        ...
        # ----------------- Stage 1
        self.init_stage = nn.Sequential(
            nn.Conv3d(in_channels, dims[0], kernel_size=(7, 7, 7), padding=(3, 3, 3)),
            Block(dim=dims[0], kernel_size=kernel_size, activation=activation)
        )
    ...
    def __repr__(self):
        return f'CellPoseNet3D[dims={self.dims}, depths={self.depths}, ' \
               f'in_channels={self.in_channels}, out_channels={self.out_channels}]'
